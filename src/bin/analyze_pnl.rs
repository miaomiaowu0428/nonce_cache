...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (mint, pnl)) in top_winners.iter().take(10).enumerate() {
        print_token_detail(i + 1, mint, pnl);
    }

    println!("\n========== 📉 Top 10 亏损代币 ==========\n");
    let top_losers = query_sorted_pnl(true).await; // 从低到高
    for (i, (mint, pnl)) in top_losers.iter().take(10).enumerate() {
        if pnl.quote_pnl >= 0 {
            break; // 已经没有亏损的了
        }
        print_token_detail(i + 1, mint, pnl);
    ...
    let mut tx_counts: Vec<_> = all_pnl
        .iter()
        .map(|(mint, pnl)| (*mint, pnl.success_tx_count))
        .collect();
    ...
    for (i, (mint, count)) in tx_counts.iter().take(5).enumerate() {
        if let Some(pnl) = all_pnl.get(mint) {
...
fn print_token_detail(rank: usize, mint: &Pubkey, pnl: &TokenPnL) {
    let quote_mint = pnl.get_quote_mint().unwrap_or_default();
    let pnl_ui = to_ui_amount(pnl.quote_pnl, &quote_mint);
    let quote_name = get_quote_name(&quote_mint);
    ...
    if pnl.is_sol_based() {
        println!(
            "{:>2}. {} {} | {:>+12.4} {} | {} 笔交易",
            rank, status, mint, pnl_ui, quote_name, pnl.success_tx_count
        );
    } else {
        let gas_ui = to_ui_amount(pnl.sol_gas_cost, &Pubkey::default());
        println!(
            "{:>2}. {} {} | {:>+12.4} {} | {} 笔交易 | gas: {:+.4} SOL",
            rank, status, mint, pnl_ui, quote_name, pnl.success_tx_count, gas_ui
        );
    }
}
